...
if [[ -z $SCRIPT_SOURCE || $SCRIPT_SOURCE == bash || $SCRIPT_SOURCE == sh ]]; then
    printf '%s\n' \
        'ERROR: This interactive script must be downloaded before it is run.' \
        'Do not use curl | bash or bash <(curl ...).' \
        'Download setup-v8.sh, then run: chmod +x setup-v8.sh && ./setup-v8.sh' >&2
    exit 1
fi
...
bootstrap_tmux() {
    ...
    [[ -t 0 && -t 1 ]] || {
        printf '%s\n' \
            ...
            'Download the script as a file and run it directly instead of using a pipe.' >&2
        exit 1
    }
